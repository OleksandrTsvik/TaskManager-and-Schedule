...
if (modalEditTask && modalDeleteTask) {
  ...
  const textareaEditDescription =
    modalFormEditTask.querySelector('#edit-description');
  ...
  function taskNotCompleted(btn, id) {
    window.location.href = `/tasks/${id}?operation=not-completed`;
  }

  function taskCompleted(btn, id) {
    window.location.href = `/tasks/${id}?operation=completed`;
  }
  ...
  function taskEdit(btn, id) {
    ...
    modalFormEditTask.action = `/tasks/${id}?operation=edit`;
  }
  ...
  function taskDelete(btn, id) {
    ...
    fieldsDeleteTask[1].innerHTML =
      description.querySelector('.card-text').innerHTML;
    fieldsDeleteTask[2].innerHTML = date.textContent.trim();

    linkDeleteTask.href = `/tasks/${id}?operation=delete`;
  }
}
