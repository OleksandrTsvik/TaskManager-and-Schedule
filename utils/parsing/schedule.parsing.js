...
async function parsingSchedule() {
    ...
    const getSubjectsData = (elements, weekday, time) => {
      ...
      for (const elem of elements) {
        ...
        subjects.push({
          ...
          teacher: deleteWhitespace(elem.children[3].textContent)
            .split(/,\s*/)
            .filter((str) => str.trim().length > 0),
          classRoom: deleteWhitespace(elem.children[2].textContent),
          groups: deleteWhitespace(elem.children[0].textContent)
            .split(/,\s*/)
            .filter((str) => str.trim().length > 0),
        });
      }
...
module.exports = parsingSchedule;
